  ...
  return (
    <header className="w-full bg-black border-b border-zinc-900 sticky top-0 z-[100] h-16 flex items-center justify-center">
      <div className="w-[96%] max-w-[1440px] h-full flex items-center justify-between px-4 relative">
        ...
        {/* LOGO */}
        <div className="flex items-center gap-4 shrink-0 z-20 bg-black pr-4">
        ...
        {/* ПРАВАЯ ЧАСТЬ */}
        <div className="flex items-center justify-end relative h-full shrink-0">
          
          {/* КОНТЕЙНЕР ПОИСКА (Абсолютное позиционирование) */}
          <div 
            className={`absolute right-0 flex items-center transition-all duration-300 ease-in-out ${
              isSearchOpen ? 'w-[calc(100vw-250px)] max-w-[400px]' : 'w-0'
            }`}
            style={{ 
              zIndex: 15,
              right: '32px' // Начинается сразу слева от лупы
            }}
          >
            <input
              ref={inputRef}
              ...
              className={`w-full bg-black border-b border-orange-600 text-sm text-white placeholder-zinc-600 py-1 outline-none transition-opacity duration-300 ${
          ...
          > 
            <a href="https://t.me/pulse_news_hub" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-6 h-6">
